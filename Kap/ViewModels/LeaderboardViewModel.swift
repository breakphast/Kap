...
class LeaderboardViewModel: ObservableObject {
    ...
    func generateUserPoints(users: [User], bets: [Bet], parlays: [Parlay], week: Int) async {
        for user in users {
            let points = await getWeeklyPoints(userID: user.id!, bets: bets, parlays: parlays, week: week)
            DispatchQueue.main.async {
                self.usersPoints[user.id!] = [week: points]
            }
        }
        DispatchQueue.main.async {
            self.rankedUsers = users
            self.rankedUsers = self.rankedUsers.sorted { user1, user2 in
